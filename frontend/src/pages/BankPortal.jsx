    ...
    <div className="min-h-screen flex flex-col p-6">
      ...
      <div className="max-w-7xl w-full mx-auto grid lg:grid-cols-4 gap-6 py-6 flex-1">
        ...
        {/* ==========================================
           ...
           ========================================== */}
        <div className="lg:col-span-1 glass-panel rounded-2xl border border-slate-800 flex flex-col overflow-hidden max-h-[750px]">
          <div className="p-4 border-b border-slate-900 bg-slate-900/25">
            <h3 className="text-sm font-bold text-white">Underwriting Pipelines</h3>
              ...
              loans.map(l => (
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-semibold text-xs text-white block truncate w-32">{l.msmeName}</span>
                    <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border ${
                      l.status === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                      (l.status === 'REJECTED' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20')
                    }`}>{l.status}</span>
                  </div>
                  
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-[10px] text-slate-500">Score: <strong className="text-slate-300">{l.score}</strong></span>
                    <span className="text-[10px] text-slate-300 font-semibold">₹{l.requestedAmount.toLocaleString('en-IN')}</span>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="lg:col-span-3 space-y-6">
          {!selectedLoan ? (
            <div className="glass-panel p-12 rounded-2xl border border-slate-800 text-center text-slate-500 flex flex-col items-center justify-center h-[500px]">
            ...
            <>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 grid md:grid-cols-4 gap-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-bl-full blur-xl"></div>
                
                <div className="md:col-span-1 flex flex-col items-center justify-center border-r border-slate-900 pr-6 text-center">
                  ...
                  <div className="relative my-3 flex items-center justify-center">
                    <div className={`w-24 h-24 rounded-full flex flex-col items-center justify-center border-4 ${
                      msmeEval.score >= 800 ? 'border-emerald-500/30 bg-emerald-500/5 glow-green' :
                      (msmeEval.score >= 650 ? 'border-primary/30 bg-primary/5 glow-blue' : 'border-red-500/30 bg-red-500/5 glow-red')
                    }`}>
                      <span className="text-2xl font-extrabold tracking-tight text-white">{msmeEval.score}</span>
                      <span className="text-[9px] text-slate-500">/ 1000</span>
                    </div>
                  </div>
                  
                  <span className={`text-[10.5px] font-bold mt-1 ${
                    msmeEval.risk_rating === 'Low Risk' ? 'text-emerald-400' :
                ...
                <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
                  <div>
                    ...
                    <strong className="text-white text-sm block mt-0.5">{msmeDetails.name}</strong>
                    <span className="text-[10px] text-slate-400">{msmeDetails.type}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">PAN / GSTIN</span>
                    <strong className="text-slate-300 block mt-0.5">{msmeDetails.pan}</strong>
                    <span className="text-[10px] text-slate-400">{msmeDetails.gstin}</span>
                  </div>
                  <div>
                  ...
                  <div className="border-t border-slate-900/60 pt-3">
                    ...
                    <strong className="text-white text-sm block mt-0.5">₹{selectedLoan.requestedAmount.toLocaleString('en-IN')}</strong>
                  </div>
                  <div className="border-t border-slate-900/60 pt-3">
                    ...
                    {copilot && (
                      <div className="space-y-4 text-[11px] leading-relaxed">
                        <div>
                          ...
                          <ul className="space-y-1 text-slate-300 list-inside list-decimal">
                        ...
                        <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
                          ...
                          <p className="text-slate-300 font-medium">{copilot.recommendation}</p>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-4 text-left">
                  ...
                  {msmeEval.stress_test && (
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                        ...
                        <strong className="text-base text-white block mt-1 font-mono">{msmeEval.stress_test.stressed_score} / 1000</strong>
                      </div>
                      <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                        <span className="block text-[10px] text-slate-500 uppercase">Runway Under Shock</span>
                        <strong className="text-base text-white block mt-1 font-mono">{msmeEval.stress_test.survival_days} Days</strong>
                      </div>
                      <div className="col-span-2 p-3 rounded-xl bg-slate-900/60 border border-slate-800 flex justify-between items-center">
                        <span className="text-[10px] text-slate-500 uppercase font-semibold">Resilience Index</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                          msmeEval.stress_test.resilience_rating === 'High Resiliency' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                          (msmeEval.stress_test.resilience_rating === 'Moderate Resiliency' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20')
                  ...
                  {msmeEval.forensics && (
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                        ...
                        <strong className={`text-base block mt-1 font-mono ${
                          msmeEval.forensics.circular_flow_probability > 40 ? 'text-red-400' : 'text-emerald-400'
                      ...
                      <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                        ...
                        <strong className={`text-base block mt-1 font-mono ${
                          msmeEval.forensics.revenue_integrity_score > 60 ? 'text-emerald-400' : 'text-red-400'
              ...
              <div className="glass-panel p-5 rounded-2xl border border-slate-800">
                ...
                <div className="grid md:grid-cols-3 gap-4 text-center">
                  ...
                  <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
                    ...
                    <strong className="text-base text-slate-300 block mt-1">{msmeEval.segment.peer_average_score}</strong>
                  </div>
                </div>
                
                <p className="text-[11px] text-slate-500 mt-4 leading-relaxed bg-slate-900/20 p-3 rounded-xl border border-slate-800/40">
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-800">
                <h4 className="text-sm font-bold text-white mb-4">Underwriting Decision Desk</h4>
                
                {selectedLoan.status !== 'PENDING' ? (
                  <div className="p-4 rounded-xl border border-slate-900 bg-slate-900/40 text-xs">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-slate-400">Loan Status</span>
                      <strong className={`px-2 py-0.5 rounded text-[10px] font-semibold border uppercase ${
                        selectedLoan.status === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'
                      }`}>{selectedLoan.status}</strong>
                    </div>
                    <p className="text-slate-400 mt-2"><span className="text-slate-500">Remarks:</span> {selectedLoan.decisionRemarks}</p>
                    {selectedLoan.status === 'APPROVED' && (
                      <p className="text-slate-400 mt-1"><span className="text-slate-500">Approved Disbursed Capital:</span> <strong>₹{selectedLoan.approvedAmount?.toLocaleString('en-IN')}</strong></p>
                    )}
                    <p className="text-[10px] text-slate-500 mt-3 border-t border-slate-900 pt-2">Authorized by {selectedLoan.officerDecisionBy} on {new Date(selectedLoan.decisionDate).toLocaleDateString()}</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="grid md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10.5px] text-slate-400 mb-1">Approved Principal Amount (₹)</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-[10.5px] text-slate-400 mb-1">Underwriting remarks / Conditions</label>
                        <input
                          type="text"
                    ...
                    <div className="flex gap-3 justify-end pt-2">
                      ...
                      <button
                        ...
                        className={`py-2.5 px-6 rounded-xl font-semibold text-xs transition flex items-center gap-1.5 ${
                          msmeEval.is_anomaly 
