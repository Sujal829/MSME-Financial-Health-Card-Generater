...
export default function MSMEPortal({ setView }) {
    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center p-6">
        <button 
          ...
          className="absolute top-6 left-6 flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-800 text-xs text-slate-400 hover:bg-slate-900 transition"
        >
          <ArrowLeft className="w-3.5 h-3.5" /> Landing Page
        </button>

        <div className="w-full max-w-md glass-panel p-8 rounded-2xl border border-slate-800 text-center">
          <div className="w-12 h-12 rounded-xl bg-blue-600/10 border border-blue-500/20 flex items-center justify-center mx-auto mb-6">
          ...
          <div className="space-y-3">
            {profiles.map(p => (
              <div 
                ...
                className="flex items-center justify-between p-4 rounded-xl border border-slate-800 hover:border-blue-500/40 hover:bg-slate-900/60 cursor-pointer text-left transition-all duration-200 group"
              >
                <div>
    ...
    return (
      <div className="min-h-screen flex justify-center items-center">
        <div className="text-center">
          ...
          <p className="text-slate-400 text-xs">Accessing credit registry records...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col p-6">
      {/* Top Navbar */}
      <nav className="flex justify-between items-center max-w-7xl w-full mx-auto pb-6 border-b border-slate-900">
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setSelectedProfileId('')}
            className="p-2 rounded-lg border border-slate-800 text-slate-400 hover:bg-slate-900 transition"
          >
          ...
          <div>
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              {profile.name}
              <span className="text-xs font-normal text-slate-500 bg-slate-900 px-2 py-0.5 rounded">
                {profile.type}
              </span>
            </h1>
        ...
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1.5 text-xs text-slate-400 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-lg">
      ...
      <div className="max-w-7xl w-full mx-auto grid lg:grid-cols-3 gap-6 py-6 flex-1">
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          {!profile.consentGranted ? (
            <div className="glass-panel p-6 rounded-2xl border border-slate-800 relative overflow-hidden">
              ...
              <div className="space-y-3 mb-6 bg-slate-900/40 p-4 rounded-xl border border-slate-800/60">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">1. GST Tax Portal (credits/debits)</span>
                  <span className="text-emerald-400 font-semibold">Ready</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">2. UPI Merchant API (P2M ledgers)</span>
                  <span className="text-emerald-400 font-semibold">Ready</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">3. ICICI/HDFC AA Bank Statements</span>
                  <span className="text-emerald-400 font-semibold">Ready</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">4. EPFO Employee Registry</span>
                  <span className="text-emerald-400 font-semibold">Ready</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-semibold text-sm transition glow-blue flex items-center justify-center gap-2"
              >
                {consentLoading ? (
                  <>
            ...
            <>
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col items-center text-center relative overflow-hidden">
                <div className="absolute top-0 right-0 w-20 h-20 bg-blue-500/10 rounded-bl-full blur-xl"></div>
                ...
                <div className="relative my-6 flex items-center justify-center">
                  ...
                  <div className={`w-36 h-36 rounded-full flex flex-col items-center justify-center border-4 ${
                    evaluation?.score >= 800 ? 'border-emerald-500/30 bg-emerald-500/5 glow-green' :
                    (evaluation?.score >= 650 ? 'border-blue-500/30 bg-blue-500/5 glow-blue' : 'border-red-500/30 bg-red-500/5 glow-red')
                  }`}>
                ...
                <div className="space-y-2 w-full mt-2">
                  <div className="flex justify-between items-center px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800">
                    <span className="text-xs text-slate-400">Risk Assessment</span>
                    <span className={`text-xs font-bold ${
                      evaluation?.risk_rating === 'Low Risk' ? 'text-emerald-400' :
                      (evaluation?.risk_rating === 'Medium Risk' ? 'text-blue-400' : 'text-red-400')
                    }`}>{evaluation?.risk_rating}</span>
                  </div>
                  <div className="flex justify-between items-center px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800">
                    <span className="text-xs text-slate-400">Default Probability</span>
                    <span className="text-xs font-bold text-white">{evaluation?.default_probability}%</span>
                  </div>
                  <div className="flex justify-between items-center px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800">
                    <span className="text-xs text-slate-400">Peer Segment</span>
                    <span className="text-xs text-slate-300 font-semibold">{evaluation?.segment.name}</span>
                  </div>
                </div>
                ...
                <div className="flex border-b border-slate-900">
                  {['breakdown', 'tips', 'resilience'].map((tab) => (
                    <button
                      ...
                      className={`flex-1 pb-2 text-xs font-semibold border-b-2 transition capitalize ${
                        leftSubTab === tab 
                    ...
                    <div className="h-[200px] w-full flex items-center justify-center">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={getRadarData()}>
                          ...
                          <PolarRadiusAxis angle={30} domain={[0, 1000]} tick={{ fontSize: 8 }} stroke="#1e293b" />
                          <Radar name="MSME" dataKey="A" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.3} />
                        </RadarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="max-h-[220px] overflow-y-auto space-y-2 pr-1">
                      {evaluation?.suggestions?.map((tip, idx) => (
                        <div key={idx} className="p-2.5 rounded-lg bg-slate-900/60 border border-slate-800 text-xs text-left">
                          <div className="flex justify-between items-center mb-1">
                            <span className="font-semibold text-white">{tip.action}</span>
                            <span className="text-[10px] text-emerald-400 font-bold bg-emerald-500/10 px-1.5 py-0.5 rounded">
                              {tip.impact}
                            </span>
                          </div>
                          <p className="text-[10px] text-slate-400 leading-normal">{tip.details}</p>
                          <div className="flex gap-2 mt-1.5 text-[9px] text-slate-500">
                            <span>Difficulty: <strong className="text-slate-400">{tip.difficulty}</strong></span>
                            <span>•</span>
                            <span>Metric: <strong className="text-slate-400">{tip.metric}</strong></span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {evaluation?.stress_test && (
                      <div className="grid grid-cols-2 gap-3 text-xs text-left">
                        ...
                        <div className="col-span-2 p-3 rounded-lg bg-slate-900/60 border border-slate-800 flex justify-between items-center">
                          <span className="text-[10px] text-slate-500">Resilience Rating</span>
                          <span className={`px-2 py-0.5 rounded font-bold text-[10px] border ${
                            evaluation.stress_test.resilience_rating === 'High Resiliency' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                            (evaluation.stress_test.resilience_rating === 'Moderate Resiliency' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20')
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800">
              ...
              {simType === 'UPI' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900/80 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-white"
                  />
                </div>
              )}
              {simType === 'GST' && (
            ...
            <button
              ...
              className="w-full py-2 rounded-xl border border-slate-800 bg-slate-900 hover:bg-slate-800/80 text-xs font-semibold text-white flex items-center justify-center gap-1.5 transition"
            >
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden">
            {/* Header Tabs */}
            <div className="flex border-b border-slate-900 bg-slate-900/40">
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex-1 py-3 px-4 text-xs font-semibold border-b-2 flex items-center justify-center gap-1.5 transition ${
                      activeTab === t.id 
                ...
                <>
                        ...
                        <table className="w-full text-left text-xs">
                          ...
                          <tbody>
                            {profile.gstData.slice(-5).map((g, i) => (
                              <tr key={i} className="border-b border-slate-900/50 hover:bg-slate-900/20 text-slate-300">
                  ...
                  {activeTab === 'upi' && (
                    <div className="space-y-6">
                      ...
                      <p className="text-[11px] text-slate-500 leading-relaxed bg-slate-900/30 border border-slate-800/40 p-3 rounded-xl">
                      ...
                      <div className="h-[220px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={getBankChartData()}>
                            ...
                            <Line type="monotone" dataKey="Credits" stroke="#3b82f6" strokeDasharray="5 5" />
                          </LineChart>
                        </ResponsiveContainer>
                      </div>
                    ...
                    <div className="space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-slate-900/30 p-4 rounded-xl border border-slate-800">
                          <h5 className="text-xs font-semibold text-slate-400 mb-3">EPFO Headcount Stability</h5>
                          <div className="h-[150px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <LineChart data={getEpfoChartData()}>
                                ...
                                <Tooltip />
                                <Line type="step" dataKey="Employees" stroke="#8b5cf6" strokeWidth={2} />
                              </LineChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
                        ...
                        {/* EPFO Payroll */}
                        <div className="bg-slate-900/30 p-4 rounded-xl border border-slate-800">
                          <h5 className="text-xs font-semibold text-slate-400 mb-3">EPFO Payroll Outgoings (in Thousands)</h5>
                          <div className="h-[150px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={getEpfoChartData()}>
                                ...
                                <YAxis stroke="#64748b" tick={{ fontSize: 9 }} />
                                <Tooltip />
                                <Bar dataKey="Payroll" fill="#8b5cf6" radius={[2, 2, 0, 0]} />
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
                      ...
                      {/* Utilities grid */}
                      <div>
                        <h5 className="text-xs font-semibold text-slate-400 mb-3">Utility Payments Compliance (Electricity Bills)</h5>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          {profile.utilityBills.slice(-4).map((u, i) => (
                            <div key={i} className="p-3 rounded-lg border border-slate-800 bg-slate-900/20 text-center">
                              <span className="block text-[10px] text-slate-500 font-semibold">{u.month}</span>
                              <span className="block text-sm font-bold text-white py-1">₹{u.amount.toLocaleString('en-IN')}</span>
                              <span className={`inline-block text-[9px] font-bold px-1.5 py-0.5 rounded ${
                                u.status.includes('Late') || u.status.includes('Penalty')
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[10.5px] text-slate-400 leading-relaxed mb-5">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-slate-300 font-bold">₹{whatIfSales.toLocaleString('en-IN')}</span>
                      </div>
                      <input
                        type="range"
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-slate-400">Cheque/ECS Bounces</span>
                        <span className={`font-bold ${whatIfBounces > 0 ? 'text-red-400' : 'text-slate-300'}`}>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-slate-400">Late Tax Filings (per 12m)</span>
                        <span className={`font-bold ${whatIfLateGst > 0 ? 'text-amber-400' : 'text-slate-300'}`}>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-slate-400">Cash Leakage (ATM withdrawals %)</span>
                        <span className="text-slate-300 font-bold">{whatIfCashWd}%</span>
                      </div>
                      <input
                        type="range"
                        min={0}
                ...
                <div className="mt-6 pt-4 border-t border-slate-900/60 flex items-center justify-between">
                  <button
                    ...
                    className="py-2 px-4 rounded-xl bg-blue-600/10 border border-blue-500/20 hover:bg-blue-600/20 text-blue-400 text-xs font-semibold transition"
                  >
              ...
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col justify-between">
                <div>
                  ...
                  {evaluation.loan_recommendation.decision === 'REJECTED' ? (
                    <div className="py-4 text-slate-500 text-xs text-center border-b border-slate-900/40">
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-3 gap-2 bg-slate-900/50 border border-slate-800/80 p-3 rounded-xl mb-4 text-center">
                        <div>
                          <span className="block text-[9px] text-slate-500 uppercase font-semibold">Max Capital</span>
                          <span className="text-sm font-bold text-white">₹{evaluation.loan_recommendation.amount_inr.toLocaleString('en-IN')}</span>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="text-sm font-bold text-white">{evaluation.loan_recommendation.tenure_months}m</span>
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  {evaluation.loan_recommendation.decision !== 'REJECTED' && !loanApplied && (
                    <form onSubmit={handleApplyLoan} className="space-y-3">
                      ...
                      <button
                        type="submit"
                        className="w-full py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs transition"
                      >
                    ...
                    <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/40 text-xs">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${
                          appliedStatus === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                          (appliedStatus === 'REJECTED' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20')
                        }`}>{appliedStatus}</span>
                      </div>
                      <div className="space-y-1.5 text-slate-400 mt-2 pt-2 border-t border-slate-900/60">
